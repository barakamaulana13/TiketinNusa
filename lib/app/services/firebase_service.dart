...
class FirebaseService extends GetxService {
  ...
  Future<UserCredential?> register(
    ...
    try {
      final cred = await _auth.createUserWithEmailAndPassword(
          email: email, password: password);
      await _simpanProfilBaru(
          cred.user!.uid, nama, email);
      return cred;
    } on FirebaseAuthException catch (e) {
  ...
  Future<UserCredential?> loginTamu() async {
    try {
      ...
      Get.snackbar('Gagal',
          'Gagal masuk sebagai tamu. Coba lagi.',
          backgroundColor: const Color(0xFF1F4529),
        ...
        .map((s) => s.docs.map((d) {
              ...
              return {
                ...
                'Tarif Hari Kerja (WNI)' :
                    data['Tarif Hari Kerja (WNI)']  ?? '0',
                'Tarif Akhir Pekan (WNI)':
                    data['Tarif Akhir Pekan (WNI)'] ?? '0',
                'Tarif Hari Kerja (WNA)' :
                    data['Tarif Hari Kerja (WNA)']  ?? '0',
                'Tarif Akhir Pekan (WNA)':
                    data['Tarif Akhir Pekan (WNA)'] ?? '0',
              };
            }).toList());
  }

  Stream<List<Map<String, dynamic>>>
      streamDestinasiByKategori(String kategori) {
    return _db
        .collection('destinasi')
  ...
  Future<void> simpanTiket(
      Map<String, dynamic> tiket) async {
    if (uid.isEmpty) return;
    await _db
